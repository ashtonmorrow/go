...
function categoryToKind(category: string | null | undefined, name: string, current: string | null, defaultToAttraction = true): string | null {
  ...
  if (/\b(restaurant|restaurante|ristorante|restoran|cafe|caffe|coffee|coffe|kavarna|bakery|bake|beigel|boulangerie|patisserie|patiseria|pasteleria|panaderia|pastry|pastries|pasteis|nata|pizza|pizzeria|sushi|maki|hamachi|ramen|noodle|dosa|donburi|teishoku|kebab|cevabdzinica|cevapi|bao|bun|kitchen|cocina|comida|cucina|krua|sapori|stuble|tacos|taqueria|brasserie|bistro|bristo|pub|tavern|brewery|brewing|bierwerk|cerveceria|cerveza|beer|taproom|grill|diner|cantina|konoba|trattoria|osteria|ostreria|parrilla|asado|bbq|barbecue|burger|cevicheria|ceviche|ostras|pezetarian|arepa|hornitos|salchicha|falafel|shawarma|empanada|burrito|gelato|ice cream|custard|rooftop|cocktail|wine bar|steakhouse|seafood|tuna|takeaway|take away|pita|street food|food truck|juice)\b|doner|burger|cacio and pepe/.test(nameText)) return 'restaurant';
  if (/\bbar\b/.test(nameText) && !/\b(old town of bar|bar,\s*montenegro)\b/.test(nameText)) return 'restaurant';
  if (/\b(airport|station|terminal|transit|metro|subway|rail|railway|ferry|cable ?car|teleferico|funicular)\b/.test(c)) return 'transit';
  if (/\b(park|garden|gardens|botanical|beach|bay|trail|reserve|forest|national park|waterfall|lido)\b/.test(c)) return 'park';
  if (/\b(store|shop|shopping|mercado|bazaar|mall|markthal|pharmacy|supermarket|boutique|tailor|barber|muji)\b|market/.test(c)) return 'shopping';
  if (/\b(museum|gallery|tourist_attraction|tourist attraction|landmark|church|cathedral|mosque|temple|synagogue|wat|ossuary|monasterio|santuario|sanctuary|place_of_worship|point_of_interest|point of interest|palace|castle|fortress|tower|aquarium|zoo|spa|thermal baths|lagoon|theme park|waterbom|windmill|college|viking centre|viking center|plaza|square|skyspace|estufa fria|house of the sea|haus des meeres|plopsaland|terra mitica|archaeological|archeological|photopoint)\b/.test(c)) return 'attraction';
